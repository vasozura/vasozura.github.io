...
async function manifestFromMidi(songId: string, url: string): Promise<ScoreManifest> {
  ...
  const notes: NoteEvent[] = midi.tracks.flatMap((track, trackIndex) => track.notes.map((note, noteIndex) => ({ id: `t${trackIndex}n${noteIndex}`, partId: `track-${trackIndex}`, measureIndex: Math.max(0, Math.floor(note.time / measureLength)), beat: 1 + note.time % measureLength / (measureLength / 4), startSeconds: note.time, durationSeconds: note.duration, midi: note.midi, velocity: note.velocity, hand: trackIndex === 0 ? "right" : "unknown" })));
...
export async function mountLearningMode(root: HTMLElement): Promise<() => void> {
  ...
  try {
    ...
    scheduler.addEventListener("frame", (event) => { const frame = (event as CustomEvent<SchedulerFrame>).detail; reliable = reliable && frame.reliable; visualizer.render(frame.active, frame.upcoming); host.querySelector<HTMLOutputElement>('[data-l="position"]')!.value = `${(frame.measure?.index ?? 0) + 1} · ${Math.max(1, Math.floor(frame.beat))}`; });
