...
const fileFields: Array<{ name: string; type: UploadFileType; column: "cover_url" | "audio_url" | "midi_url" | "musicxml_url" | "score_pdf_url" | "source_project_url" }> = [
...
async function saveForm(form: HTMLFormElement, existing: Song, message: HTMLElement): Promise<void> {
  ...
  try {
    ...
    const resources: Partial<Record<(typeof fileFields)[number]["column"], string>> = {};
    for (const field of fileFields) {
      ...
      resources[field.column] = uploaded.publicUrl;
...
export async function mountAdmin(root: HTMLElement): Promise<void> {
  ...
  const renderDashboard = (): void => {
    ...
    root.querySelectorAll<HTMLButtonElement>("[data-admin-publish]").forEach((button) => button.addEventListener("click", async () => { await setSongStatus(button.dataset.adminPublish ?? "", button.dataset.status === "published" ? "published" : "draft"); songs = await loadAdminSongs(); renderDashboard(); }));
    root.querySelectorAll<HTMLButtonElement>("[data-admin-delete]").forEach((button) => button.addEventListener("click", async () => { const song = songs.find((entry) => entry.id === button.dataset.adminDelete); if (!song || !window.confirm(`Delete “${song.title.ka ?? song.slug}”? Uploaded files remain versioned until removed from Storage.`)) return; await deleteSong(song.id); songs = await loadAdminSongs(); renderDashboard(); }));
