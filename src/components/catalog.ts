...
export function renderCatalog(songs: Song[], language: Language): string {
  ...
  const labels = language === "ka"
    ? { search: "ძებნა", searchPlaceholder: "სათაური, ავტორი ან პოეტი", language: "ენა", lyricist: "პოეტი / ტექსტის ავტორი", composer: "კომპოზიტორი", difficulty: "სირთულე", resource: "რესურსი", all: "ყველა", audio: "MP3 აუდიო", midi: "MIDI", score: "MusicXML", pdf: "PDF ნოტები", lyrics: "ტექსტი", reset: "ფილტრების გასუფთავება", count: "ნაპოვნია" }
    : { search: "Search", searchPlaceholder: "Title, credit, composer or poet", language: "Language", lyricist: "Poet / lyricist", composer: "Composer", difficulty: "Difficulty", resource: "Resource", all: "All", audio: "MP3 audio", midi: "MIDI", score: "MusicXML", pdf: "PDF score", lyrics: "Lyrics", reset: "Clear filters", count: "Showing" };
  ...
  return `
    <section class="music shell" id="music" aria-labelledby="music-title">
      ...
      <form class="catalog-filters" id="catalog-filters" role="search">
        ...
        <label><span>${labels.lyricist}</span><input id="catalog-lyricist" type="search" placeholder="${labels.all}" autocomplete="off" /></label>
        <label><span>${labels.composer}</span><input id="catalog-composer" type="search" placeholder="${labels.all}" autocomplete="off" /></label>
      ...
      <p class="catalog-loading" id="catalog-loading" role="status" aria-live="polite" hidden></p>
      ...
      <button class="catalog-more" type="button" data-catalog-more hidden>${language === "ka" ? "მეტის ჩატვირთვა" : "Load more"}</button>
