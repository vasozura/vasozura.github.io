...
export function renderSongDetail(song: Song, language: Language): string {
  ...
  return `
    <main class="song-detail shell" id="main-content" tabindex="-1">
      ...
      ${song.musicXmlUrl ? `<section class="song-resource score-panel" id="interactive-score" data-musicxml-url="${escapeHtml(song.musicXmlUrl)}" data-midi-url="${escapeHtml(song.midiUrl ?? "")}" data-bpm="${song.bpm ?? 120}" aria-labelledby="score-title"><h2 id="score-title">${labels.score}</h2><p class="score-status" aria-live="polite">Loading score…</p><div class="score-controls"></div><div class="score-canvas"></div><div class="midi-controls"></div><div class="piano-keyboard" aria-label="Piano visualization"></div></section>` : ""}
